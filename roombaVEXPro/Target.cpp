...
void Target::eval()
{
	...
	if (target_distance < TARGET_RADIUS) {		// reached the current target
		if (targetIndex == targetCnt-1) {
			...
			printf("Finished mission, reached target at X_target: %0.1f Y_target: %0.1f with X_pos: %0.1f Y_pos: %0.1f, theta: %0.1f",
	...
	if (heading_error > 45)
		...
		rotRqst = -1 - (int)((TO_MM * target_distance)/4.0);	// radius in mm is 1/4 of distance to target
	else if (heading_error < -2)
		rotRqst = 1 + (int)((TO_MM * target_distance)/4.0);
	else
		rotRqst = 0;
}
