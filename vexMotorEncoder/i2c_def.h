...
typedef struct
{ 
  u8  deviceAddr;       // the "8-bit" address of device. Divided by 2 when passed to libI2C
  bool  direction;        // set to I2C_READ or I2C_WRITE from libI2C.h
  u8 bytesToWrite;     //# of bytes to write
  int writeIndex;       //write buffer index
  u8 writeBufr[256];
  u8 writeComplete;    //write has completed
  int bytesToRead;      //# of bytes to read
  u8 readIndex;        //read buffer index
  u8 readBufr[256];
  u8 readBufrReady;  
  u8 status;           //1=Ok, 0=Timeout Error
} I2C_Record;
