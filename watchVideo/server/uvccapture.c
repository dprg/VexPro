...
typedef struct {
	volatile int doCapture;	// set to 1 by cmdHandler, cleared by main when capture is complete
...
void startVideoSrvr()
{
	...
	while (run) {
		...
		if (ctrl.doCapture == 1) {
			...
			if (verbose >=1)
				fprintf (stderr, "converted image to luminance or rgb in buffer at 0x%x\n", (int)ctrl.imgArray);
